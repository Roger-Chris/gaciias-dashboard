...
export default function DashboardClient({ initialData }: DashboardClientProps) {
  const [kpis, setKpis] = useState<DashboardKPI[]>(initialData?.kpis || []);
  const [syncTime, setSyncTime] = useState<string>(initialData?.timestamp || "");
  const [sheetLastUpdated, setSheetLastUpdated] = useState<string>(initialData?.lastUpdated || "");
  const [dataSource, setDataSource] = useState<string>(initialData?.source || "sheet");
  const [loading, setLoading] = useState<boolean>(!initialData);
  const [syncing, setSyncing] = useState<boolean>(false);
  const [emailSending, setEmailSending] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(initialData ? null : "Could not retrieve Google Sheets data");
